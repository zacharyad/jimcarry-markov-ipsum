...
function jimRambles(lines = 10, words = 10, str = corpus) {
  ...
  for (let i = 0; i < lines; i++) {
    ...
    for (let j = 0; j < wordsPerLine; j++) {
      if (prevWord.length < 4 && j === wordsPerLine - 1) continue;
...
function strToObj(str) {
  ...
  for (let i = 0; i < arrayOfWords.length; i++) {
    ...
    if (
      arrayOfWords[i] === ' ' ||
      arrayOfWords[i] === '-' ||
      (arrayOfWords[i].length === 1 && singleLetterWords.indexOf() !== -1)
    ) {
...
console.log(`${jimRambles(args[0], args[1])}`);
